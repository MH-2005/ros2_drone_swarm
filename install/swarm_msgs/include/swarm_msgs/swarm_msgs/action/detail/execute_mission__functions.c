// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from swarm_msgs:action/ExecuteMission.idl
// generated code does not contain a copyright notice
#include "swarm_msgs/action/detail/execute_mission__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `mission_type`
// Member `path_file`
// Member `formation_type`
#include "rosidl_runtime_c/string_functions.h"
// Member `waypoints`
#include "geometry_msgs/msg/detail/point__functions.h"

bool
swarm_msgs__action__ExecuteMission_Goal__init(swarm_msgs__action__ExecuteMission_Goal * msg)
{
  if (!msg) {
    return false;
  }
  // mission_type
  if (!rosidl_runtime_c__String__init(&msg->mission_type)) {
    swarm_msgs__action__ExecuteMission_Goal__fini(msg);
    return false;
  }
  // path_file
  if (!rosidl_runtime_c__String__init(&msg->path_file)) {
    swarm_msgs__action__ExecuteMission_Goal__fini(msg);
    return false;
  }
  // formation_spacing
  // formation_type
  if (!rosidl_runtime_c__String__init(&msg->formation_type)) {
    swarm_msgs__action__ExecuteMission_Goal__fini(msg);
    return false;
  }
  // waypoints
  if (!geometry_msgs__msg__Point__Sequence__init(&msg->waypoints, 0)) {
    swarm_msgs__action__ExecuteMission_Goal__fini(msg);
    return false;
  }
  // max_duration
  return true;
}

void
swarm_msgs__action__ExecuteMission_Goal__fini(swarm_msgs__action__ExecuteMission_Goal * msg)
{
  if (!msg) {
    return;
  }
  // mission_type
  rosidl_runtime_c__String__fini(&msg->mission_type);
  // path_file
  rosidl_runtime_c__String__fini(&msg->path_file);
  // formation_spacing
  // formation_type
  rosidl_runtime_c__String__fini(&msg->formation_type);
  // waypoints
  geometry_msgs__msg__Point__Sequence__fini(&msg->waypoints);
  // max_duration
}

bool
swarm_msgs__action__ExecuteMission_Goal__are_equal(const swarm_msgs__action__ExecuteMission_Goal * lhs, const swarm_msgs__action__ExecuteMission_Goal * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // mission_type
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->mission_type), &(rhs->mission_type)))
  {
    return false;
  }
  // path_file
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->path_file), &(rhs->path_file)))
  {
    return false;
  }
  // formation_spacing
  if (lhs->formation_spacing != rhs->formation_spacing) {
    return false;
  }
  // formation_type
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->formation_type), &(rhs->formation_type)))
  {
    return false;
  }
  // waypoints
  if (!geometry_msgs__msg__Point__Sequence__are_equal(
      &(lhs->waypoints), &(rhs->waypoints)))
  {
    return false;
  }
  // max_duration
  if (lhs->max_duration != rhs->max_duration) {
    return false;
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_Goal__copy(
  const swarm_msgs__action__ExecuteMission_Goal * input,
  swarm_msgs__action__ExecuteMission_Goal * output)
{
  if (!input || !output) {
    return false;
  }
  // mission_type
  if (!rosidl_runtime_c__String__copy(
      &(input->mission_type), &(output->mission_type)))
  {
    return false;
  }
  // path_file
  if (!rosidl_runtime_c__String__copy(
      &(input->path_file), &(output->path_file)))
  {
    return false;
  }
  // formation_spacing
  output->formation_spacing = input->formation_spacing;
  // formation_type
  if (!rosidl_runtime_c__String__copy(
      &(input->formation_type), &(output->formation_type)))
  {
    return false;
  }
  // waypoints
  if (!geometry_msgs__msg__Point__Sequence__copy(
      &(input->waypoints), &(output->waypoints)))
  {
    return false;
  }
  // max_duration
  output->max_duration = input->max_duration;
  return true;
}

swarm_msgs__action__ExecuteMission_Goal *
swarm_msgs__action__ExecuteMission_Goal__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_Goal * msg = (swarm_msgs__action__ExecuteMission_Goal *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_Goal), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(swarm_msgs__action__ExecuteMission_Goal));
  bool success = swarm_msgs__action__ExecuteMission_Goal__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
swarm_msgs__action__ExecuteMission_Goal__destroy(swarm_msgs__action__ExecuteMission_Goal * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    swarm_msgs__action__ExecuteMission_Goal__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
swarm_msgs__action__ExecuteMission_Goal__Sequence__init(swarm_msgs__action__ExecuteMission_Goal__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_Goal * data = NULL;

  if (size) {
    data = (swarm_msgs__action__ExecuteMission_Goal *)allocator.zero_allocate(size, sizeof(swarm_msgs__action__ExecuteMission_Goal), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = swarm_msgs__action__ExecuteMission_Goal__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        swarm_msgs__action__ExecuteMission_Goal__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
swarm_msgs__action__ExecuteMission_Goal__Sequence__fini(swarm_msgs__action__ExecuteMission_Goal__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      swarm_msgs__action__ExecuteMission_Goal__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

swarm_msgs__action__ExecuteMission_Goal__Sequence *
swarm_msgs__action__ExecuteMission_Goal__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_Goal__Sequence * array = (swarm_msgs__action__ExecuteMission_Goal__Sequence *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_Goal__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = swarm_msgs__action__ExecuteMission_Goal__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
swarm_msgs__action__ExecuteMission_Goal__Sequence__destroy(swarm_msgs__action__ExecuteMission_Goal__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    swarm_msgs__action__ExecuteMission_Goal__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
swarm_msgs__action__ExecuteMission_Goal__Sequence__are_equal(const swarm_msgs__action__ExecuteMission_Goal__Sequence * lhs, const swarm_msgs__action__ExecuteMission_Goal__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_Goal__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_Goal__Sequence__copy(
  const swarm_msgs__action__ExecuteMission_Goal__Sequence * input,
  swarm_msgs__action__ExecuteMission_Goal__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(swarm_msgs__action__ExecuteMission_Goal);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    swarm_msgs__action__ExecuteMission_Goal * data =
      (swarm_msgs__action__ExecuteMission_Goal *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!swarm_msgs__action__ExecuteMission_Goal__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          swarm_msgs__action__ExecuteMission_Goal__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_Goal__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `final_status`
// already included above
// #include "rosidl_runtime_c/string_functions.h"
// Member `final_positions`
#include "geometry_msgs/msg/detail/pose__functions.h"

bool
swarm_msgs__action__ExecuteMission_Result__init(swarm_msgs__action__ExecuteMission_Result * msg)
{
  if (!msg) {
    return false;
  }
  // success
  // total_time
  // waypoints_completed
  // final_status
  if (!rosidl_runtime_c__String__init(&msg->final_status)) {
    swarm_msgs__action__ExecuteMission_Result__fini(msg);
    return false;
  }
  // final_positions
  if (!geometry_msgs__msg__Pose__Sequence__init(&msg->final_positions, 0)) {
    swarm_msgs__action__ExecuteMission_Result__fini(msg);
    return false;
  }
  return true;
}

void
swarm_msgs__action__ExecuteMission_Result__fini(swarm_msgs__action__ExecuteMission_Result * msg)
{
  if (!msg) {
    return;
  }
  // success
  // total_time
  // waypoints_completed
  // final_status
  rosidl_runtime_c__String__fini(&msg->final_status);
  // final_positions
  geometry_msgs__msg__Pose__Sequence__fini(&msg->final_positions);
}

bool
swarm_msgs__action__ExecuteMission_Result__are_equal(const swarm_msgs__action__ExecuteMission_Result * lhs, const swarm_msgs__action__ExecuteMission_Result * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // success
  if (lhs->success != rhs->success) {
    return false;
  }
  // total_time
  if (lhs->total_time != rhs->total_time) {
    return false;
  }
  // waypoints_completed
  if (lhs->waypoints_completed != rhs->waypoints_completed) {
    return false;
  }
  // final_status
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->final_status), &(rhs->final_status)))
  {
    return false;
  }
  // final_positions
  if (!geometry_msgs__msg__Pose__Sequence__are_equal(
      &(lhs->final_positions), &(rhs->final_positions)))
  {
    return false;
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_Result__copy(
  const swarm_msgs__action__ExecuteMission_Result * input,
  swarm_msgs__action__ExecuteMission_Result * output)
{
  if (!input || !output) {
    return false;
  }
  // success
  output->success = input->success;
  // total_time
  output->total_time = input->total_time;
  // waypoints_completed
  output->waypoints_completed = input->waypoints_completed;
  // final_status
  if (!rosidl_runtime_c__String__copy(
      &(input->final_status), &(output->final_status)))
  {
    return false;
  }
  // final_positions
  if (!geometry_msgs__msg__Pose__Sequence__copy(
      &(input->final_positions), &(output->final_positions)))
  {
    return false;
  }
  return true;
}

swarm_msgs__action__ExecuteMission_Result *
swarm_msgs__action__ExecuteMission_Result__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_Result * msg = (swarm_msgs__action__ExecuteMission_Result *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_Result), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(swarm_msgs__action__ExecuteMission_Result));
  bool success = swarm_msgs__action__ExecuteMission_Result__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
swarm_msgs__action__ExecuteMission_Result__destroy(swarm_msgs__action__ExecuteMission_Result * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    swarm_msgs__action__ExecuteMission_Result__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
swarm_msgs__action__ExecuteMission_Result__Sequence__init(swarm_msgs__action__ExecuteMission_Result__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_Result * data = NULL;

  if (size) {
    data = (swarm_msgs__action__ExecuteMission_Result *)allocator.zero_allocate(size, sizeof(swarm_msgs__action__ExecuteMission_Result), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = swarm_msgs__action__ExecuteMission_Result__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        swarm_msgs__action__ExecuteMission_Result__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
swarm_msgs__action__ExecuteMission_Result__Sequence__fini(swarm_msgs__action__ExecuteMission_Result__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      swarm_msgs__action__ExecuteMission_Result__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

swarm_msgs__action__ExecuteMission_Result__Sequence *
swarm_msgs__action__ExecuteMission_Result__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_Result__Sequence * array = (swarm_msgs__action__ExecuteMission_Result__Sequence *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_Result__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = swarm_msgs__action__ExecuteMission_Result__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
swarm_msgs__action__ExecuteMission_Result__Sequence__destroy(swarm_msgs__action__ExecuteMission_Result__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    swarm_msgs__action__ExecuteMission_Result__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
swarm_msgs__action__ExecuteMission_Result__Sequence__are_equal(const swarm_msgs__action__ExecuteMission_Result__Sequence * lhs, const swarm_msgs__action__ExecuteMission_Result__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_Result__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_Result__Sequence__copy(
  const swarm_msgs__action__ExecuteMission_Result__Sequence * input,
  swarm_msgs__action__ExecuteMission_Result__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(swarm_msgs__action__ExecuteMission_Result);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    swarm_msgs__action__ExecuteMission_Result * data =
      (swarm_msgs__action__ExecuteMission_Result *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!swarm_msgs__action__ExecuteMission_Result__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          swarm_msgs__action__ExecuteMission_Result__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_Result__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `leader_pose`
// Member `follower_poses`
// already included above
// #include "geometry_msgs/msg/detail/pose__functions.h"
// Member `status_message`
// already included above
// #include "rosidl_runtime_c/string_functions.h"

bool
swarm_msgs__action__ExecuteMission_Feedback__init(swarm_msgs__action__ExecuteMission_Feedback * msg)
{
  if (!msg) {
    return false;
  }
  // current_waypoint
  // distance_to_target
  // elapsed_time
  // leader_pose
  if (!geometry_msgs__msg__Pose__init(&msg->leader_pose)) {
    swarm_msgs__action__ExecuteMission_Feedback__fini(msg);
    return false;
  }
  // follower_poses
  if (!geometry_msgs__msg__Pose__Sequence__init(&msg->follower_poses, 0)) {
    swarm_msgs__action__ExecuteMission_Feedback__fini(msg);
    return false;
  }
  // status_message
  if (!rosidl_runtime_c__String__init(&msg->status_message)) {
    swarm_msgs__action__ExecuteMission_Feedback__fini(msg);
    return false;
  }
  return true;
}

void
swarm_msgs__action__ExecuteMission_Feedback__fini(swarm_msgs__action__ExecuteMission_Feedback * msg)
{
  if (!msg) {
    return;
  }
  // current_waypoint
  // distance_to_target
  // elapsed_time
  // leader_pose
  geometry_msgs__msg__Pose__fini(&msg->leader_pose);
  // follower_poses
  geometry_msgs__msg__Pose__Sequence__fini(&msg->follower_poses);
  // status_message
  rosidl_runtime_c__String__fini(&msg->status_message);
}

bool
swarm_msgs__action__ExecuteMission_Feedback__are_equal(const swarm_msgs__action__ExecuteMission_Feedback * lhs, const swarm_msgs__action__ExecuteMission_Feedback * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // current_waypoint
  if (lhs->current_waypoint != rhs->current_waypoint) {
    return false;
  }
  // distance_to_target
  if (lhs->distance_to_target != rhs->distance_to_target) {
    return false;
  }
  // elapsed_time
  if (lhs->elapsed_time != rhs->elapsed_time) {
    return false;
  }
  // leader_pose
  if (!geometry_msgs__msg__Pose__are_equal(
      &(lhs->leader_pose), &(rhs->leader_pose)))
  {
    return false;
  }
  // follower_poses
  if (!geometry_msgs__msg__Pose__Sequence__are_equal(
      &(lhs->follower_poses), &(rhs->follower_poses)))
  {
    return false;
  }
  // status_message
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->status_message), &(rhs->status_message)))
  {
    return false;
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_Feedback__copy(
  const swarm_msgs__action__ExecuteMission_Feedback * input,
  swarm_msgs__action__ExecuteMission_Feedback * output)
{
  if (!input || !output) {
    return false;
  }
  // current_waypoint
  output->current_waypoint = input->current_waypoint;
  // distance_to_target
  output->distance_to_target = input->distance_to_target;
  // elapsed_time
  output->elapsed_time = input->elapsed_time;
  // leader_pose
  if (!geometry_msgs__msg__Pose__copy(
      &(input->leader_pose), &(output->leader_pose)))
  {
    return false;
  }
  // follower_poses
  if (!geometry_msgs__msg__Pose__Sequence__copy(
      &(input->follower_poses), &(output->follower_poses)))
  {
    return false;
  }
  // status_message
  if (!rosidl_runtime_c__String__copy(
      &(input->status_message), &(output->status_message)))
  {
    return false;
  }
  return true;
}

swarm_msgs__action__ExecuteMission_Feedback *
swarm_msgs__action__ExecuteMission_Feedback__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_Feedback * msg = (swarm_msgs__action__ExecuteMission_Feedback *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_Feedback), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(swarm_msgs__action__ExecuteMission_Feedback));
  bool success = swarm_msgs__action__ExecuteMission_Feedback__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
swarm_msgs__action__ExecuteMission_Feedback__destroy(swarm_msgs__action__ExecuteMission_Feedback * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    swarm_msgs__action__ExecuteMission_Feedback__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
swarm_msgs__action__ExecuteMission_Feedback__Sequence__init(swarm_msgs__action__ExecuteMission_Feedback__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_Feedback * data = NULL;

  if (size) {
    data = (swarm_msgs__action__ExecuteMission_Feedback *)allocator.zero_allocate(size, sizeof(swarm_msgs__action__ExecuteMission_Feedback), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = swarm_msgs__action__ExecuteMission_Feedback__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        swarm_msgs__action__ExecuteMission_Feedback__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
swarm_msgs__action__ExecuteMission_Feedback__Sequence__fini(swarm_msgs__action__ExecuteMission_Feedback__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      swarm_msgs__action__ExecuteMission_Feedback__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

swarm_msgs__action__ExecuteMission_Feedback__Sequence *
swarm_msgs__action__ExecuteMission_Feedback__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_Feedback__Sequence * array = (swarm_msgs__action__ExecuteMission_Feedback__Sequence *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_Feedback__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = swarm_msgs__action__ExecuteMission_Feedback__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
swarm_msgs__action__ExecuteMission_Feedback__Sequence__destroy(swarm_msgs__action__ExecuteMission_Feedback__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    swarm_msgs__action__ExecuteMission_Feedback__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
swarm_msgs__action__ExecuteMission_Feedback__Sequence__are_equal(const swarm_msgs__action__ExecuteMission_Feedback__Sequence * lhs, const swarm_msgs__action__ExecuteMission_Feedback__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_Feedback__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_Feedback__Sequence__copy(
  const swarm_msgs__action__ExecuteMission_Feedback__Sequence * input,
  swarm_msgs__action__ExecuteMission_Feedback__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(swarm_msgs__action__ExecuteMission_Feedback);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    swarm_msgs__action__ExecuteMission_Feedback * data =
      (swarm_msgs__action__ExecuteMission_Feedback *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!swarm_msgs__action__ExecuteMission_Feedback__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          swarm_msgs__action__ExecuteMission_Feedback__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_Feedback__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
#include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `goal`
// already included above
// #include "swarm_msgs/action/detail/execute_mission__functions.h"

bool
swarm_msgs__action__ExecuteMission_SendGoal_Request__init(swarm_msgs__action__ExecuteMission_SendGoal_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    swarm_msgs__action__ExecuteMission_SendGoal_Request__fini(msg);
    return false;
  }
  // goal
  if (!swarm_msgs__action__ExecuteMission_Goal__init(&msg->goal)) {
    swarm_msgs__action__ExecuteMission_SendGoal_Request__fini(msg);
    return false;
  }
  return true;
}

void
swarm_msgs__action__ExecuteMission_SendGoal_Request__fini(swarm_msgs__action__ExecuteMission_SendGoal_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // goal
  swarm_msgs__action__ExecuteMission_Goal__fini(&msg->goal);
}

bool
swarm_msgs__action__ExecuteMission_SendGoal_Request__are_equal(const swarm_msgs__action__ExecuteMission_SendGoal_Request * lhs, const swarm_msgs__action__ExecuteMission_SendGoal_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // goal
  if (!swarm_msgs__action__ExecuteMission_Goal__are_equal(
      &(lhs->goal), &(rhs->goal)))
  {
    return false;
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_SendGoal_Request__copy(
  const swarm_msgs__action__ExecuteMission_SendGoal_Request * input,
  swarm_msgs__action__ExecuteMission_SendGoal_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // goal
  if (!swarm_msgs__action__ExecuteMission_Goal__copy(
      &(input->goal), &(output->goal)))
  {
    return false;
  }
  return true;
}

swarm_msgs__action__ExecuteMission_SendGoal_Request *
swarm_msgs__action__ExecuteMission_SendGoal_Request__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_SendGoal_Request * msg = (swarm_msgs__action__ExecuteMission_SendGoal_Request *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_SendGoal_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(swarm_msgs__action__ExecuteMission_SendGoal_Request));
  bool success = swarm_msgs__action__ExecuteMission_SendGoal_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
swarm_msgs__action__ExecuteMission_SendGoal_Request__destroy(swarm_msgs__action__ExecuteMission_SendGoal_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    swarm_msgs__action__ExecuteMission_SendGoal_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence__init(swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_SendGoal_Request * data = NULL;

  if (size) {
    data = (swarm_msgs__action__ExecuteMission_SendGoal_Request *)allocator.zero_allocate(size, sizeof(swarm_msgs__action__ExecuteMission_SendGoal_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = swarm_msgs__action__ExecuteMission_SendGoal_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        swarm_msgs__action__ExecuteMission_SendGoal_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence__fini(swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      swarm_msgs__action__ExecuteMission_SendGoal_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence *
swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence * array = (swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence__destroy(swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence__are_equal(const swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence * lhs, const swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_SendGoal_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence__copy(
  const swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence * input,
  swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(swarm_msgs__action__ExecuteMission_SendGoal_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    swarm_msgs__action__ExecuteMission_SendGoal_Request * data =
      (swarm_msgs__action__ExecuteMission_SendGoal_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!swarm_msgs__action__ExecuteMission_SendGoal_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          swarm_msgs__action__ExecuteMission_SendGoal_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_SendGoal_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `stamp`
#include "builtin_interfaces/msg/detail/time__functions.h"

bool
swarm_msgs__action__ExecuteMission_SendGoal_Response__init(swarm_msgs__action__ExecuteMission_SendGoal_Response * msg)
{
  if (!msg) {
    return false;
  }
  // accepted
  // stamp
  if (!builtin_interfaces__msg__Time__init(&msg->stamp)) {
    swarm_msgs__action__ExecuteMission_SendGoal_Response__fini(msg);
    return false;
  }
  return true;
}

void
swarm_msgs__action__ExecuteMission_SendGoal_Response__fini(swarm_msgs__action__ExecuteMission_SendGoal_Response * msg)
{
  if (!msg) {
    return;
  }
  // accepted
  // stamp
  builtin_interfaces__msg__Time__fini(&msg->stamp);
}

bool
swarm_msgs__action__ExecuteMission_SendGoal_Response__are_equal(const swarm_msgs__action__ExecuteMission_SendGoal_Response * lhs, const swarm_msgs__action__ExecuteMission_SendGoal_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // accepted
  if (lhs->accepted != rhs->accepted) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__are_equal(
      &(lhs->stamp), &(rhs->stamp)))
  {
    return false;
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_SendGoal_Response__copy(
  const swarm_msgs__action__ExecuteMission_SendGoal_Response * input,
  swarm_msgs__action__ExecuteMission_SendGoal_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // accepted
  output->accepted = input->accepted;
  // stamp
  if (!builtin_interfaces__msg__Time__copy(
      &(input->stamp), &(output->stamp)))
  {
    return false;
  }
  return true;
}

swarm_msgs__action__ExecuteMission_SendGoal_Response *
swarm_msgs__action__ExecuteMission_SendGoal_Response__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_SendGoal_Response * msg = (swarm_msgs__action__ExecuteMission_SendGoal_Response *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_SendGoal_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(swarm_msgs__action__ExecuteMission_SendGoal_Response));
  bool success = swarm_msgs__action__ExecuteMission_SendGoal_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
swarm_msgs__action__ExecuteMission_SendGoal_Response__destroy(swarm_msgs__action__ExecuteMission_SendGoal_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    swarm_msgs__action__ExecuteMission_SendGoal_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence__init(swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_SendGoal_Response * data = NULL;

  if (size) {
    data = (swarm_msgs__action__ExecuteMission_SendGoal_Response *)allocator.zero_allocate(size, sizeof(swarm_msgs__action__ExecuteMission_SendGoal_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = swarm_msgs__action__ExecuteMission_SendGoal_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        swarm_msgs__action__ExecuteMission_SendGoal_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence__fini(swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      swarm_msgs__action__ExecuteMission_SendGoal_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence *
swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence * array = (swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence__destroy(swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence__are_equal(const swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence * lhs, const swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_SendGoal_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence__copy(
  const swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence * input,
  swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(swarm_msgs__action__ExecuteMission_SendGoal_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    swarm_msgs__action__ExecuteMission_SendGoal_Response * data =
      (swarm_msgs__action__ExecuteMission_SendGoal_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!swarm_msgs__action__ExecuteMission_SendGoal_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          swarm_msgs__action__ExecuteMission_SendGoal_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_SendGoal_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `info`
#include "service_msgs/msg/detail/service_event_info__functions.h"
// Member `request`
// Member `response`
// already included above
// #include "swarm_msgs/action/detail/execute_mission__functions.h"

bool
swarm_msgs__action__ExecuteMission_SendGoal_Event__init(swarm_msgs__action__ExecuteMission_SendGoal_Event * msg)
{
  if (!msg) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__init(&msg->info)) {
    swarm_msgs__action__ExecuteMission_SendGoal_Event__fini(msg);
    return false;
  }
  // request
  if (!swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence__init(&msg->request, 0)) {
    swarm_msgs__action__ExecuteMission_SendGoal_Event__fini(msg);
    return false;
  }
  // response
  if (!swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence__init(&msg->response, 0)) {
    swarm_msgs__action__ExecuteMission_SendGoal_Event__fini(msg);
    return false;
  }
  return true;
}

void
swarm_msgs__action__ExecuteMission_SendGoal_Event__fini(swarm_msgs__action__ExecuteMission_SendGoal_Event * msg)
{
  if (!msg) {
    return;
  }
  // info
  service_msgs__msg__ServiceEventInfo__fini(&msg->info);
  // request
  swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence__fini(&msg->request);
  // response
  swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence__fini(&msg->response);
}

bool
swarm_msgs__action__ExecuteMission_SendGoal_Event__are_equal(const swarm_msgs__action__ExecuteMission_SendGoal_Event * lhs, const swarm_msgs__action__ExecuteMission_SendGoal_Event * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__are_equal(
      &(lhs->info), &(rhs->info)))
  {
    return false;
  }
  // request
  if (!swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence__are_equal(
      &(lhs->request), &(rhs->request)))
  {
    return false;
  }
  // response
  if (!swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence__are_equal(
      &(lhs->response), &(rhs->response)))
  {
    return false;
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_SendGoal_Event__copy(
  const swarm_msgs__action__ExecuteMission_SendGoal_Event * input,
  swarm_msgs__action__ExecuteMission_SendGoal_Event * output)
{
  if (!input || !output) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__copy(
      &(input->info), &(output->info)))
  {
    return false;
  }
  // request
  if (!swarm_msgs__action__ExecuteMission_SendGoal_Request__Sequence__copy(
      &(input->request), &(output->request)))
  {
    return false;
  }
  // response
  if (!swarm_msgs__action__ExecuteMission_SendGoal_Response__Sequence__copy(
      &(input->response), &(output->response)))
  {
    return false;
  }
  return true;
}

swarm_msgs__action__ExecuteMission_SendGoal_Event *
swarm_msgs__action__ExecuteMission_SendGoal_Event__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_SendGoal_Event * msg = (swarm_msgs__action__ExecuteMission_SendGoal_Event *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_SendGoal_Event), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(swarm_msgs__action__ExecuteMission_SendGoal_Event));
  bool success = swarm_msgs__action__ExecuteMission_SendGoal_Event__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
swarm_msgs__action__ExecuteMission_SendGoal_Event__destroy(swarm_msgs__action__ExecuteMission_SendGoal_Event * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    swarm_msgs__action__ExecuteMission_SendGoal_Event__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence__init(swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_SendGoal_Event * data = NULL;

  if (size) {
    data = (swarm_msgs__action__ExecuteMission_SendGoal_Event *)allocator.zero_allocate(size, sizeof(swarm_msgs__action__ExecuteMission_SendGoal_Event), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = swarm_msgs__action__ExecuteMission_SendGoal_Event__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        swarm_msgs__action__ExecuteMission_SendGoal_Event__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence__fini(swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      swarm_msgs__action__ExecuteMission_SendGoal_Event__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence *
swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence * array = (swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence__destroy(swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence__are_equal(const swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence * lhs, const swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_SendGoal_Event__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence__copy(
  const swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence * input,
  swarm_msgs__action__ExecuteMission_SendGoal_Event__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(swarm_msgs__action__ExecuteMission_SendGoal_Event);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    swarm_msgs__action__ExecuteMission_SendGoal_Event * data =
      (swarm_msgs__action__ExecuteMission_SendGoal_Event *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!swarm_msgs__action__ExecuteMission_SendGoal_Event__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          swarm_msgs__action__ExecuteMission_SendGoal_Event__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_SendGoal_Event__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"

bool
swarm_msgs__action__ExecuteMission_GetResult_Request__init(swarm_msgs__action__ExecuteMission_GetResult_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    swarm_msgs__action__ExecuteMission_GetResult_Request__fini(msg);
    return false;
  }
  return true;
}

void
swarm_msgs__action__ExecuteMission_GetResult_Request__fini(swarm_msgs__action__ExecuteMission_GetResult_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
}

bool
swarm_msgs__action__ExecuteMission_GetResult_Request__are_equal(const swarm_msgs__action__ExecuteMission_GetResult_Request * lhs, const swarm_msgs__action__ExecuteMission_GetResult_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_GetResult_Request__copy(
  const swarm_msgs__action__ExecuteMission_GetResult_Request * input,
  swarm_msgs__action__ExecuteMission_GetResult_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  return true;
}

swarm_msgs__action__ExecuteMission_GetResult_Request *
swarm_msgs__action__ExecuteMission_GetResult_Request__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_GetResult_Request * msg = (swarm_msgs__action__ExecuteMission_GetResult_Request *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_GetResult_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(swarm_msgs__action__ExecuteMission_GetResult_Request));
  bool success = swarm_msgs__action__ExecuteMission_GetResult_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
swarm_msgs__action__ExecuteMission_GetResult_Request__destroy(swarm_msgs__action__ExecuteMission_GetResult_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    swarm_msgs__action__ExecuteMission_GetResult_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence__init(swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_GetResult_Request * data = NULL;

  if (size) {
    data = (swarm_msgs__action__ExecuteMission_GetResult_Request *)allocator.zero_allocate(size, sizeof(swarm_msgs__action__ExecuteMission_GetResult_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = swarm_msgs__action__ExecuteMission_GetResult_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        swarm_msgs__action__ExecuteMission_GetResult_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence__fini(swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      swarm_msgs__action__ExecuteMission_GetResult_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence *
swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence * array = (swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence__destroy(swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence__are_equal(const swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence * lhs, const swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_GetResult_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence__copy(
  const swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence * input,
  swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(swarm_msgs__action__ExecuteMission_GetResult_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    swarm_msgs__action__ExecuteMission_GetResult_Request * data =
      (swarm_msgs__action__ExecuteMission_GetResult_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!swarm_msgs__action__ExecuteMission_GetResult_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          swarm_msgs__action__ExecuteMission_GetResult_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_GetResult_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `result`
// already included above
// #include "swarm_msgs/action/detail/execute_mission__functions.h"

bool
swarm_msgs__action__ExecuteMission_GetResult_Response__init(swarm_msgs__action__ExecuteMission_GetResult_Response * msg)
{
  if (!msg) {
    return false;
  }
  // status
  // result
  if (!swarm_msgs__action__ExecuteMission_Result__init(&msg->result)) {
    swarm_msgs__action__ExecuteMission_GetResult_Response__fini(msg);
    return false;
  }
  return true;
}

void
swarm_msgs__action__ExecuteMission_GetResult_Response__fini(swarm_msgs__action__ExecuteMission_GetResult_Response * msg)
{
  if (!msg) {
    return;
  }
  // status
  // result
  swarm_msgs__action__ExecuteMission_Result__fini(&msg->result);
}

bool
swarm_msgs__action__ExecuteMission_GetResult_Response__are_equal(const swarm_msgs__action__ExecuteMission_GetResult_Response * lhs, const swarm_msgs__action__ExecuteMission_GetResult_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  // result
  if (!swarm_msgs__action__ExecuteMission_Result__are_equal(
      &(lhs->result), &(rhs->result)))
  {
    return false;
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_GetResult_Response__copy(
  const swarm_msgs__action__ExecuteMission_GetResult_Response * input,
  swarm_msgs__action__ExecuteMission_GetResult_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // status
  output->status = input->status;
  // result
  if (!swarm_msgs__action__ExecuteMission_Result__copy(
      &(input->result), &(output->result)))
  {
    return false;
  }
  return true;
}

swarm_msgs__action__ExecuteMission_GetResult_Response *
swarm_msgs__action__ExecuteMission_GetResult_Response__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_GetResult_Response * msg = (swarm_msgs__action__ExecuteMission_GetResult_Response *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_GetResult_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(swarm_msgs__action__ExecuteMission_GetResult_Response));
  bool success = swarm_msgs__action__ExecuteMission_GetResult_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
swarm_msgs__action__ExecuteMission_GetResult_Response__destroy(swarm_msgs__action__ExecuteMission_GetResult_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    swarm_msgs__action__ExecuteMission_GetResult_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence__init(swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_GetResult_Response * data = NULL;

  if (size) {
    data = (swarm_msgs__action__ExecuteMission_GetResult_Response *)allocator.zero_allocate(size, sizeof(swarm_msgs__action__ExecuteMission_GetResult_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = swarm_msgs__action__ExecuteMission_GetResult_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        swarm_msgs__action__ExecuteMission_GetResult_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence__fini(swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      swarm_msgs__action__ExecuteMission_GetResult_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence *
swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence * array = (swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence__destroy(swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence__are_equal(const swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence * lhs, const swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_GetResult_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence__copy(
  const swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence * input,
  swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(swarm_msgs__action__ExecuteMission_GetResult_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    swarm_msgs__action__ExecuteMission_GetResult_Response * data =
      (swarm_msgs__action__ExecuteMission_GetResult_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!swarm_msgs__action__ExecuteMission_GetResult_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          swarm_msgs__action__ExecuteMission_GetResult_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_GetResult_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `info`
// already included above
// #include "service_msgs/msg/detail/service_event_info__functions.h"
// Member `request`
// Member `response`
// already included above
// #include "swarm_msgs/action/detail/execute_mission__functions.h"

bool
swarm_msgs__action__ExecuteMission_GetResult_Event__init(swarm_msgs__action__ExecuteMission_GetResult_Event * msg)
{
  if (!msg) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__init(&msg->info)) {
    swarm_msgs__action__ExecuteMission_GetResult_Event__fini(msg);
    return false;
  }
  // request
  if (!swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence__init(&msg->request, 0)) {
    swarm_msgs__action__ExecuteMission_GetResult_Event__fini(msg);
    return false;
  }
  // response
  if (!swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence__init(&msg->response, 0)) {
    swarm_msgs__action__ExecuteMission_GetResult_Event__fini(msg);
    return false;
  }
  return true;
}

void
swarm_msgs__action__ExecuteMission_GetResult_Event__fini(swarm_msgs__action__ExecuteMission_GetResult_Event * msg)
{
  if (!msg) {
    return;
  }
  // info
  service_msgs__msg__ServiceEventInfo__fini(&msg->info);
  // request
  swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence__fini(&msg->request);
  // response
  swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence__fini(&msg->response);
}

bool
swarm_msgs__action__ExecuteMission_GetResult_Event__are_equal(const swarm_msgs__action__ExecuteMission_GetResult_Event * lhs, const swarm_msgs__action__ExecuteMission_GetResult_Event * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__are_equal(
      &(lhs->info), &(rhs->info)))
  {
    return false;
  }
  // request
  if (!swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence__are_equal(
      &(lhs->request), &(rhs->request)))
  {
    return false;
  }
  // response
  if (!swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence__are_equal(
      &(lhs->response), &(rhs->response)))
  {
    return false;
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_GetResult_Event__copy(
  const swarm_msgs__action__ExecuteMission_GetResult_Event * input,
  swarm_msgs__action__ExecuteMission_GetResult_Event * output)
{
  if (!input || !output) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__copy(
      &(input->info), &(output->info)))
  {
    return false;
  }
  // request
  if (!swarm_msgs__action__ExecuteMission_GetResult_Request__Sequence__copy(
      &(input->request), &(output->request)))
  {
    return false;
  }
  // response
  if (!swarm_msgs__action__ExecuteMission_GetResult_Response__Sequence__copy(
      &(input->response), &(output->response)))
  {
    return false;
  }
  return true;
}

swarm_msgs__action__ExecuteMission_GetResult_Event *
swarm_msgs__action__ExecuteMission_GetResult_Event__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_GetResult_Event * msg = (swarm_msgs__action__ExecuteMission_GetResult_Event *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_GetResult_Event), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(swarm_msgs__action__ExecuteMission_GetResult_Event));
  bool success = swarm_msgs__action__ExecuteMission_GetResult_Event__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
swarm_msgs__action__ExecuteMission_GetResult_Event__destroy(swarm_msgs__action__ExecuteMission_GetResult_Event * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    swarm_msgs__action__ExecuteMission_GetResult_Event__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence__init(swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_GetResult_Event * data = NULL;

  if (size) {
    data = (swarm_msgs__action__ExecuteMission_GetResult_Event *)allocator.zero_allocate(size, sizeof(swarm_msgs__action__ExecuteMission_GetResult_Event), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = swarm_msgs__action__ExecuteMission_GetResult_Event__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        swarm_msgs__action__ExecuteMission_GetResult_Event__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence__fini(swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      swarm_msgs__action__ExecuteMission_GetResult_Event__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence *
swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence * array = (swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence__destroy(swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence__are_equal(const swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence * lhs, const swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_GetResult_Event__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence__copy(
  const swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence * input,
  swarm_msgs__action__ExecuteMission_GetResult_Event__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(swarm_msgs__action__ExecuteMission_GetResult_Event);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    swarm_msgs__action__ExecuteMission_GetResult_Event * data =
      (swarm_msgs__action__ExecuteMission_GetResult_Event *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!swarm_msgs__action__ExecuteMission_GetResult_Event__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          swarm_msgs__action__ExecuteMission_GetResult_Event__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_GetResult_Event__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `feedback`
// already included above
// #include "swarm_msgs/action/detail/execute_mission__functions.h"

bool
swarm_msgs__action__ExecuteMission_FeedbackMessage__init(swarm_msgs__action__ExecuteMission_FeedbackMessage * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    swarm_msgs__action__ExecuteMission_FeedbackMessage__fini(msg);
    return false;
  }
  // feedback
  if (!swarm_msgs__action__ExecuteMission_Feedback__init(&msg->feedback)) {
    swarm_msgs__action__ExecuteMission_FeedbackMessage__fini(msg);
    return false;
  }
  return true;
}

void
swarm_msgs__action__ExecuteMission_FeedbackMessage__fini(swarm_msgs__action__ExecuteMission_FeedbackMessage * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // feedback
  swarm_msgs__action__ExecuteMission_Feedback__fini(&msg->feedback);
}

bool
swarm_msgs__action__ExecuteMission_FeedbackMessage__are_equal(const swarm_msgs__action__ExecuteMission_FeedbackMessage * lhs, const swarm_msgs__action__ExecuteMission_FeedbackMessage * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // feedback
  if (!swarm_msgs__action__ExecuteMission_Feedback__are_equal(
      &(lhs->feedback), &(rhs->feedback)))
  {
    return false;
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_FeedbackMessage__copy(
  const swarm_msgs__action__ExecuteMission_FeedbackMessage * input,
  swarm_msgs__action__ExecuteMission_FeedbackMessage * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // feedback
  if (!swarm_msgs__action__ExecuteMission_Feedback__copy(
      &(input->feedback), &(output->feedback)))
  {
    return false;
  }
  return true;
}

swarm_msgs__action__ExecuteMission_FeedbackMessage *
swarm_msgs__action__ExecuteMission_FeedbackMessage__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_FeedbackMessage * msg = (swarm_msgs__action__ExecuteMission_FeedbackMessage *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_FeedbackMessage), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(swarm_msgs__action__ExecuteMission_FeedbackMessage));
  bool success = swarm_msgs__action__ExecuteMission_FeedbackMessage__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
swarm_msgs__action__ExecuteMission_FeedbackMessage__destroy(swarm_msgs__action__ExecuteMission_FeedbackMessage * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    swarm_msgs__action__ExecuteMission_FeedbackMessage__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence__init(swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_FeedbackMessage * data = NULL;

  if (size) {
    data = (swarm_msgs__action__ExecuteMission_FeedbackMessage *)allocator.zero_allocate(size, sizeof(swarm_msgs__action__ExecuteMission_FeedbackMessage), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = swarm_msgs__action__ExecuteMission_FeedbackMessage__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        swarm_msgs__action__ExecuteMission_FeedbackMessage__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence__fini(swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      swarm_msgs__action__ExecuteMission_FeedbackMessage__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence *
swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence * array = (swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence *)allocator.allocate(sizeof(swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence__destroy(swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence__are_equal(const swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence * lhs, const swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_FeedbackMessage__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence__copy(
  const swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence * input,
  swarm_msgs__action__ExecuteMission_FeedbackMessage__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(swarm_msgs__action__ExecuteMission_FeedbackMessage);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    swarm_msgs__action__ExecuteMission_FeedbackMessage * data =
      (swarm_msgs__action__ExecuteMission_FeedbackMessage *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!swarm_msgs__action__ExecuteMission_FeedbackMessage__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          swarm_msgs__action__ExecuteMission_FeedbackMessage__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!swarm_msgs__action__ExecuteMission_FeedbackMessage__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
